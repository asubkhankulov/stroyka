...
if(preg_match('~^mesto/$~ms', $Path, $q)) {
	...
	$list = MySQL::fetchAllArray('SELECT mt.*,(SELECT count(*) FROM uchastok where mesto_id = mt.id) as kol FROM mesto mt WHERE mt.type = 0 ORDER BY name');

	//$smarty->assign('today',date('d.m.Y',time()));
	$smarty->assign('list',$list);

	$smarty->assign('INC_PAGE','mesto.tpl');

}
elseif(preg_match('~^mesto/([0-9]+)/$~ms', $Path, $q)) {
	...
	if (isset($_POST['action'])) {
		...
		if (!isset($_POST['id'])) {		
			...
			$id = intval($_POST['id']);
			MySQL::query("UPDATE uchastok SET name = '".$_POST['name']."', mesto_id= '".$mesto_id."', s = '".$s."', d = '".$d."', shir = '".$shir."',
							septik = '".$_POST['septik']."', voda = '".$_POST['voda']."', note = '".$_POST['note']."'
			WHERE id = {$id}");			
	...
	$smarty->assign('INC_PAGE','mesto_uchastki.tpl');	 	
}
elseif(preg_match('~^mesto/edit/([0-9]+)/$~ms', $Path, $q)) {
	...
	$smarty->assign('info',$info);
	$smarty->assign('INC_PAGE','mesto_uchastki_edit.tpl');	 	
}
