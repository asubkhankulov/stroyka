	...
	<div class="well well-sm">
		<a href="/stroyka/">Стройка</a> / <a href="/stroyka/view/{$info.uchastok_id}/">в {$info.poselok} на участке {$info.uchastok}</a> / {$info.object_type} <a href="/stroyka/step/{$stroyka_id}/{$doma_id}/">{$info.object}</a> / <a href="/stroyka/works/{$stroyka_id}/{$step_id}/{$doma_id}/">{$info.step}</a> / {$info.work} <a href="/stroyka/materials/{$stroyka_id}/{$work_id}/{$step_id}/{$doma_id}/" title="Обновить страницу"><i class="fa fa-refresh" aria-hidden="true"></i></a> - cписок материалов:
	</div>                
  ...
  <div class="modal-dialog">
    <div class="modal-content modal-sm">
 	...
 	<div class="row">
		...
		<form class="form-horizontal text-primary" action="/stroyka/materials/{$stroyka_id}/{$work_id}/{$step_id}/{$doma_id}/" method="POST">
	...
	$(document).ready(function() {            
			...
			UpdateField = function(field_id, obj) {
	            ...
	            $.post(url,
	            {
	                'action': 'update_real_doma_materials',
	                'id': field_id,
	                'val1': val1,
	                'val2': val2
	                ...
	                case 'ok':
	                	$(field).tooltip('show');
	                	window.location = "/stroyka/materials/{$stroyka_id}/{$work_id}/{$step_id}/{$doma_id}/";
	                    break
	                default:
	                	$(field).tooltip(data);
	            }
		    ...
		    $('#Del').click(function() {
			    ...
			    if (cnt_id != 0) {
			        ...
			        $.post(url,
			        {
			            'action': 'del_real_doma_material_work',
			            'id': cnt_id
			        }, checkTime = function(data) {
			        	 //console.log(data);
			        	 if (data == 'ok')  { 
