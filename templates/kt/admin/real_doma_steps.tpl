<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
	<div class="well well-sm">
		<a href="/stroyka/">Стройка</a> / <a href="/stroyka/view/{$info.uchastok_id}/">в {$info.poselok} на участке {$info.uchastok}</a> / {$info.object_type} <a href="/stroyka/step/{$stroyka_id}/{$doma_id}/">{$info.object}</a>  - cписок этапов:
	</div>                
	...
	<div class="row">
		...
		<div><br/>
		  ...
		  <!-- Tab panes --> {* Внутренние Внешние Сети Благоустройство Документация *}
		  <div class="tab-content">
		    ...
		    <br>
                     ...
                     <thead>
                        ...
                        {section name=cus loop=$list}
                            <tr id="cnt_{$list[cus].id}">
                                ...
                                <td id="cntval_{$list[cus].id}"><a href="/stroyka/works/{$stroyka_id}/{$list[cus].step_id}/{$doma_id}/">{$list[cus].name}</a></td>
	    		...
	    		<br>
					...
					<form class="form-horizontal text-primary" action="/stroyka/step/{$stroyka_id}/{$doma_id}/" method="POST">
					...
					<form class="form-horizontal text-primary" action="/stroyka/step/{$stroyka_id}/{$doma_id}/" method="POST">
					  ...
					  <div class="form-group">
					    ...
					    <div class="col-sm-8">
				           	<select class="form-control" name="step_id" id="input3">
						      {if !empty($list_steps)}{section name=cus loop=$list_steps}
						      	{if $list_steps[cus].type == 'Внешние'}
								  <option value="{$list_steps[cus].id}">{$list_steps[cus].name}</option>
								{/if}
							  {/section}{/if}
							</select>
				...
				<div class="panel panel-warning">
                 ...
                 <table class="table table-bordered" id="htmltable1">
                     <thead>
                        ...
                        {assign var="all_days" value=0}
                        {section name=cus loop=$list}
                        	{if $list[cus].type == 'Благоустройство'}
                            <tr id="cnt_{$list[cus].id}">
                             	<td>{{$smarty.section.cus.iteration}}</td>
                                <td id="cntval_{$list[cus].id}"><a href="/stroyka/works/{$stroyka_id}/{$list[cus].step_id}/{$doma_id}/">{$list[cus].name}</a></td>
                                <td><input type="text" class="form-control" name="v_step" value="{$list[cus].v_step}" id="{$list[cus].id}_v_step"></td>
					...
					<form class="form-horizontal text-primary" action="/stroyka/step/{$stroyka_id}/{$doma_id}/" method="POST">
					  ...
					  <div class="form-group">
					    ...
					    <div class="col-sm-8">
				           	<select class="form-control" name="step_id" id="input3">
						      {if !empty($list_steps)}{section name=cus loop=$list_steps}
						      	{if $list_steps[cus].type == 'Благоустройство'}
								  <option value="{$list_steps[cus].id}">{$list_steps[cus].name}</option>
								{/if}
							  {/section}{/if}
							</select>
					...
					<form class="form-horizontal text-primary" action="/stroyka/step/{$stroyka_id}/{$doma_id}/" method="POST">
					  ...
					  <div class="form-group">
					    ...
					    <div class="col-sm-8">
				           	<select class="form-control" name="step_id" id="input3">
						      {if !empty($list_steps)}{section name=cus loop=$list_steps}
						      	{if $list_steps[cus].type == 'Документация'}
								  <option value="{$list_steps[cus].id}">{$list_steps[cus].name}</option>
								{/if}
							  {/section}{/if}
							</select>
